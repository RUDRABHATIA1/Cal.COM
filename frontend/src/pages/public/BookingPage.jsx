...
export default function BookingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white selection:bg-white/20">
      ...
      <div className="relative max-w-6xl mx-auto min-h-screen flex items-center justify-center p-4 md:p-8">
        <motion.div 
          layout
          initial="initial" animate="animate" variants={containerVariants}
          className="w-full bg-[#111111]/80 backdrop-blur-3xl border border-white/10 rounded-[32px] shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[640px]"
        >
          {/* ── Left Column: Host Details ────────────────────── */}
          <div className="w-full md:w-[380px] p-10 border-b md:border-b-0 md:border-r border-white/5 flex flex-col bg-white/[0.02]">
            {rescheduleId && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8 p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex gap-3">
                <AlertCircle className="w-5 h-5 text-amber-500 shrink-0" />
                <div className="text-[13px] text-amber-200/80 leading-relaxed font-medium">
            ...
            <div className="mb-10">
              <div className="flex items-center gap-4 mb-8">
                <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-xl font-black uppercase text-white shadow-inner">
                  {data.host.name.charAt(0)}
                </div>
                <div>
                  <p className="text-white/30 text-[10px] font-black uppercase tracking-[0.15em] mb-0.5">{data.host.username}</p>
                  <p className="text-xl font-bold text-white/95">{data.host.name}</p>
                </div>
              </div>
              <h1 className="text-3xl font-bold bg-gradient-to-br from-white to-white/40 bg-clip-text text-transparent mb-5 leading-[1.15] font-cal tracking-tight">
                {data.eventType.title}
              </h1>
              <p className="text-white/50 text-[15px] leading-relaxed line-clamp-4">
            ...
            <div className="space-y-6 flex-1">
              <div className="flex items-center gap-4 text-white/70">
                <div className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><Clock className="w-4 h-4" /></div>
                <span className="text-[15px] font-semibold">{data.eventType.length} minutes</span>
              </div>
              <div className="flex items-center gap-4 text-white/70">
                <div className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 grid place-items-center"><Globe className="w-4 h-4" /></div>
                <span className="text-[15px] font-semibold">{availability?.timezone || 'Universal Time'}</span>
              </div>
            </div>

            {step > 1 && step < 4 && (
              <button onClick={() => setStep(step - 1)} className="mt-12 group flex items-center gap-3 text-white/30 hover:text-white transition-all text-xs font-bold uppercase tracking-wider">
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div key="step1" variants={stepVariants} initial="initial" animate="animate" exit="exit" className="h-full flex flex-col">
                  <div className="mb-10">
                    <h2 className="text-2xl font-bold text-white font-cal tracking-tight">Select a date</h2>
                    <p className="text-white/40 text-[14px] mt-1.5 font-medium">Choose a day that works best for you.</p>
                  </div>
                  ...
                  <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} onClick={() => setStep(2)} className="w-full mt-10 py-5 bg-white text-black rounded-2xl font-black text-sm tracking-wide shadow-[0_10px_40px_rgba(255,255,255,0.1)]">Select Time</motion.button>
                </motion.div>
              )}
              ...
              {step === 2 && (
                <motion.div key="step2" variants={stepVariants} initial="initial" animate="animate" exit="exit" className="h-full flex flex-col">
                  <div className="mb-10 flex items-end justify-between">
                    <div>
                      <h2 className="text-2xl font-bold text-white font-cal tracking-tight">Select a time</h2>
                      <p className="text-white/40 text-[14px] mt-1.5 font-bold uppercase tracking-widest">{format(selectedDate, 'EEEE, MMM do')}</p>
                    </div>
                    {slotsLoading && <Loader2 className="w-5 h-5 animate-spin text-white/40" />}
                  </div>
                  
                  <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.05 } } }} className="grid grid-cols-2 gap-3 overflow-y-auto pr-3 flex-1 scroll-smooth no-scrollbar" style={{ maxHeight: '420px' }}>
                    {availableSlots.length > 0 ? (
                      ...
                      <div className="col-span-2 py-20 text-center flex flex-col items-center gap-6 border border-dashed border-white/10 rounded-[32px] bg-white/[0.02]">
                        <div className="w-14 h-14 rounded-full bg-white/5 grid place-items-center"><CalendarIcon className="w-6 h-6 text-white/20" /></div>
                        <p className="text-white/20 font-bold tracking-wide uppercase text-[11px]">No slots available</p>
                      </div>
                    )}
                  </motion.div>
                  ...
                  <motion.button disabled={!selectedTime} whileHover={selectedTime ? { scale: 1.01 } : {}} whileTap={selectedTime ? { scale: 0.99 } : {}}
                    ...
                    className="w-full mt-10 py-5 bg-white text-black rounded-2xl font-black text-sm tracking-wide disabled:opacity-20 shadow-[0_10px_40px_rgba(255,255,255,0.1)]"
                  >
              ...
              {step === 3 && (
                <motion.div key="step3" variants={stepVariants} initial="initial" animate="animate" exit="exit" className="max-w-md mx-auto w-full flex flex-col h-full">
                  <div className="mb-12 text-center">
                    <h2 className="text-3xl font-bold text-white font-cal mb-3">Almost there</h2>
                    <p className="text-white/40 text-[15px] font-medium leading-relaxed">Please verify your details below to confirm.</p>
                  </div>
                  ...
                  <form onSubmit={handleBooking} className="space-y-6 flex-1">
                    <div className="space-y-4">
                      ...
                      <div className="relative">
                        ...
                        <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} placeholder="Additional notes" rows={5}
                          className="w-full pl-14 pr-5 py-5 bg-white/[0.04] border border-white/10 rounded-[24px] focus:border-white/30 focus:bg-white/[0.06] outline-none transition-all resize-none text-[15px]" />
                      </div>
                    </div>

                    <button type="submit" disabled={isSubmitting} className="w-full mt-auto py-5 bg-white text-black rounded-2xl font-black shadow-2xl flex items-center justify-center gap-3">
              ...
              {step === 4 && (
                <motion.div key="step4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="h-full flex flex-col items-center justify-center text-center p-10">
                  <div className="w-24 h-24 bg-green-500/10 border border-green-500/20 rounded-[40px] flex items-center justify-center text-green-400 mb-12 shadow-[0_0_50px_rgba(52,211,153,0.1)]">
                    <CheckCircle2 className="w-12 h-12" />
                  </div>
                  <h2 className="text-4xl font-bold text-white font-cal mb-5 tracking-tight">{rescheduleId ? 'Successfully Rescheduled!' : 'Perfectly Timed!'}</h2>
                  <p className="text-white/50 mb-12 text-[17px] leading-relaxed max-w-sm">
                  ...
                  <div className="w-full max-w-sm bg-white/[0.04] border border-white/5 rounded-[32px] p-8 text-left space-y-5">
                    <SummaryRow label="Host" val={data.host.name} />
                    <SummaryRow label="New Time" val={`${format(selectedDate, 'MMM do')}, ${selectedTime}`} />
                  </div>
                  <button onClick={() => window.location.href = '/dashboard/bookings'} className="mt-12 text-[10px] font-black uppercase tracking-[0.2em] text-white/20 hover:text-white transition-all">Back to Dashboard</button>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
      ...
      <style>{`
        ...
        .premium-calendar .react-calendar__navigation { margin-bottom: 2.5rem; background: rgba(255,255,255,0.03); border-radius: 20px; padding: 6px; }
        .premium-calendar .react-calendar__navigation button { color: #fff; font-weight: 800; border-radius: 14px; padding: 12px 0; font-family: "Cal Sans", sans-serif; }
        .premium-calendar .react-calendar__navigation button:hover { background: rgba(255,255,255,0.08); }
        .premium-calendar .react-calendar__month-view__weekdays { font-size: 0.7rem; color: rgba(255,255,255,0.2); font-weight: 900; text-transform: uppercase; margin-bottom: 1.5rem; letter-spacing: 0.1em; }
        .premium-calendar .react-calendar__month-view__weekdays__weekday { display: flex; justify-content: center; }
        .premium-calendar .react-calendar__tile { color: rgba(255,255,255,0.6); aspect-ratio: 1; border-radius: 16px; font-weight: 700; font-size: 0.95rem; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); margin-bottom: 4px; }
